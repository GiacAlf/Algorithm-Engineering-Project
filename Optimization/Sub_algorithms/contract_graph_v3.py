...
def contract_graph(graph, partition):
    """
    Contracts a graph based on the provided partition. Nodes in the same subset of the partition
    are contracted into a single supernode. Edges between different supernodes are preserved.

    :param graph: The original NetworKit graph
    :param partition: A NetworKit Partition object that defines the subsets for contraction
    :return: A new contracted NetworKit graph
    """
    ...
    assert contracted_graph.numberOfEdges() <= graph.numberOfEdges(), (
        "Contracted graph has more edges than the original graph"
    )
...
if __name__ == "__main__":
    ...
    assert contracted.numberOfEdges() == graph.numberOfEdges(), (
        "Contracted graph should have the same number of edges as the original"
    )
